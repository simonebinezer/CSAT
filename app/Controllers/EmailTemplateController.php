...
class EmailTemplateController extends BaseController
{
    public function index()
    {
        ...
        //  $from= session()->get("survey_email");
        return view('admin/emailtemplate', ["getSurvey" => $getSurvey, "externalList" => $externalList, "from" => $from]);
    }
    ...
    public function sendEmail()
    {
        ...
        if ($this->request->getMethod() == 'post' && !($this->request->getPost("email_id"))) {
            ...
            if (!$this->validate($rules, $errors)) {
                ...
                return view('admin/emailtemplate', ["validation" => $this->validator, "getSurvey" => $getSurvey, "externalList" => $externalList, "from" => $from]);
            } else {
        ...
        return view('admin/emailtemplate', ["getSurvey" => $getSurvey, "externalList" => $externalList, "to" => $this->request->getPost("email_id"), "from" => $from]);
    }
    ...
    public function ScheduleMail($postdata, $userId, $emailList, $tenant, $isCsat = false)
    {
        $model = new ExternalcontactsModel();
        $contactlist = $model->whereIn('email_id', $emailList)->where('status', 1)->findAll();
        $contact = [];
        ...
        if ($isCsat == true) {
            $template = "csat/email-template-survey";
        } else {
            $template = "template/email-template-survey";
        }
        for ($i = 0; $i < count($contactlist); $i++) {


            $contact = $contactlist[$i];
            $template = view($template, ["userId" => $userId, "postdata" => $postdata, "contactdata" => $contact, "tenantdata" => $tenant]); //, "surveyrandom" => $surveyrandom]);
            ...
            $mail->addAddress($contact ["email_id"]);
            $mail->isHTML(true);
            $response = $mail->send();
            $data[$i]['tenant_id'] = $tenant_id;
            $data[$i]['survey_id'] = $survey_id;
            ...
            $data[$i]['from_name'] = $from_name;
            $data[$i]['mail'] = $template;
            $data[$i]['updated_by'] = session()->get("id");
        }
